...
* @Last Modified time : 2016-11-24T16:05:43+08:00
 -->
...
<div style="background: #fff;">
    ...
    <div class="wrap">
        <div class="tab_header">
            <h2>分析维度：</h2>
            <div class='radio_box'>
                <!-- <ui-radio name="tab" value="1" :model.sync="tab" label="离职类型" checked></ui-radio> -->
                <ui-radio name="tab" value="2" :model.sync="tab" label="离职年龄" checked></ui-radio>
                <ui-radio name="tab" value="3" :model.sync="tab" label="离职司龄"></ui-radio>
            </div>
        </div>
        <div class="tab_content_list">
            <!-- <div v-show="tab == 1" class="tab_content_1">
                <ui-report v-ref:report   :column-total="true" :data-list="state.leaveEmployeeWorkAgeAnalysisReportDataList" :titles="titles" :multi-titles="multiTitles" :row-total="rowTotal"  :group-infos="groupColumns" :total-ignore='totalIgnore' :progress-show="processShow"
                    :lorder="lorder">
                </ui-report>
            </div> -->
            <div v-if="tab == 2" class="tab_content_2">
                <ui-report v-ref:report :column-total="true" :data-list="state.leaveEmployeeAgeAnalysisReportDataList" :titles="titles2" :multi-titles="multiTitles" :row-total="rowTotal2" :group-infos="groupColumns" :total-ignore='totalIgnore' :progress-show="processShow"
                    :lorder="lorder" :td-height="70">
                </ui-report>
            </div>
            <div v-if="tab == 3" class="tab_content_3">
                <ui-report v-ref:report :column-total="true" :data-list="state.leaveEmployeeWorkAgeAnalysisReportDataList" :titles="titles3" :multi-titles="multiTitles" :row-total="rowTotal3" :group-infos="groupColumns" :total-ignore='totalIgnore' :progress-show="processShow"
                    :lorder="lorder" :td-height="70">
                </ui-report>
            </div>
        </div>
        ...
        loadData(userName) {
            ...
            if (store.actions.checkConditions()) {
                if (self.state.flag) {
                    ...
                    store.actions.loadLeaveEmployeeAgeAnalysisReport(function(data) {
                        self.rowTotal = true;
                    ...
                    store.actions.setLoadParams(this.timeCondition, userName);
                    store.actions.loadLeaveEmployeeWorkAgeAnalysisReport(function(data) {
                        self.rowTotal = true;
                        self.processShow = false;
                        self.$nextTick(() => {
                            self.$refs.report.refresh();
            ...
            if (store.actions.checkConditions()) {
                if (this.state.flag) {
                    ...
                    this.$nextTick(function() {
                        ...
                        if (this.tab == '2') {
                            window.location.href = Vue.__Basepath + 'acewill/common/exportLeaveEmployeeAgeAnalysisReport.htm?' + param;
                        } else if (this.tab == '3') {
                            window.location.href = Vue.__Basepath + 'acewill/common/exportLeaveEmployeeWorkAgeAnalysisReport.htm?' + param;
                        }
                    })
...
.tab_header .ui-radio-border {
    background-color: none !important;
    border: 2px solid #fff !important;
}

.tab_header .ui-radio-inner-dot {
