...
export default {
  data() {
      return {
        ...
        url: 'api.Dashboard.getDayCheckWork',
        ...
        fields: [{
          ...
          formater: function(value, data, index) {
            if (!isNaN(data.legalLimitTime) && !isNaN(data.hoursForDay)) {
              return ((data.legalLimitTime - 0) - (data.hoursForDay - 0)).toFixed(2);
            } else {
              return 'N/A';
            }
          },
    ...
    methods: {
      exportExcel() {
        ...
        this.$root.downloadFile('acewill/common/card/exportdaycheckworkwarning.htm', param);
      },
      searchAtt() {
        this.refreshTable();
      },
