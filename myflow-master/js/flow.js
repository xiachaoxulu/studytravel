...
var flow = {
    ...
    main: {
            ...
            var start = function() {
                ...
                move = function(dx, dy) {
                    ...
                    if (this.type == 'circle') {
                        ...
                        this.attrs.rex = x, this.attrs.rey = y;
                    }
                    this.text.attr({
                        x: this.text.ox + dx,
                        y: this.text.oy + dy
        ...
        lineReset: function(l) {
                ...
                console.log(distanceOP,distanceED);
                if (path.length >= 3) {
                    $(l[0]).attr({ 'd': 'M' + op[num[0]][0] + ',' + op[num[0]][1] + 'L' + path[1].split(',')[0] + ',' + path[1].split(',')[1] + 'L' + ed[num[1]][0] + ',' + ed[num[1]][1] });
                    l.text.attr({ 'x': parseInt(path[1].split(',')[0]) + 25, 'y': parseInt(path[1].split(',')[1]) - 15 });
                } else {
                	console.log('M' + op[num[0]][0] + ',' + op[num[0]][1] + 'L' + ed[num[1]][0] + ',' + ed[num[1]][1]);
        ...
        content: function(el) {
            ...
            console.log(p.type);
            if (p.type == "path") {
            	console.log(p.text.attrs.condition);
                for (var i in p.text.attrs.condition) {
                    condition += '<div>' + '<label >条件</label>' + '<input class="span2" type="text" value="' + p.text.attrs.condition[i] + '"/>' + '</div>';
                }
                condition += '<a class="add" href="#">+</a>' + '<a class="del" href="#">-</a>';
            } else if (p.origin == 'branch') {
                for (var i in p.text.attrs.condition) {
                    condition += '<div>' + '<label >判断</label>' + '<input class="span2" type="text" value="' + p.text.attrs.condition[i] + '"/>' + '</div>';
                }
                condition += '<a class="add" href="#">+</a>' + '<a class="del" href="#">-</a>';
            } else if (p.type == 'rect') {
                for (var i in p.text.attrs.condition) {
                    condition += '<div>' + '<input class="key span1" type="text" value="' + i + '"/>' + '<span>:</span>' + '<input class="value" type="text" value="' + p.text.attrs.condition[i] + '"/>' + '</div>';
                }
                condition += '<a class="add" href="#">+</a>' + '<a class="del" href="#">-</a>';
            }
            var html = '<div class="content" op="' + p.id + '">' + '<h4>属性</h4>' + '<div class="form">' + '<div>' + '<label for="name">显示</label>' + '<input class="span2" type="text" id="name" value="' + p.text.attr('text') + '"/>' + '</div>' + '<div>' + '<label for="description">描述</label>' + '<textarea class="span2" name="description" id="description" cols="20" rows="2">' + p.text.attr('description') + '</textarea>' + '</div>' + '<div class="condition">' + condition + '</div>' + '<div class="butt">' + '<input type="button" class="btn btn-success" id="confirm" value="确定"/>' + '<input type="button" class="btn" id="cancle" value="取消"/>' + '</div>' + '<div class="butt">' + '<input type="button" class="btn btn-danger span2" id="delete" value="删除此元素"/>' + '</div>' + '</div>' + '</div>';
            $('body').append(html);
            $('.content .add').unbind('click', null);
            $('.content .add').bind('click', function() {
                if (p.type == "path") {
                    var html = '<div>' + '<label >条件</label>' + '<input class="span2" type="text" value=""/>' + '</div>'
                    $(this).before(html)
                } else if (p.origin == 'branch') {
                    var html = '<div>' + '<label >判断</label>' + '<input class="span2" type="text" value=""/>' + '</div>'
                    $(this).before(html);
                } else if (p.type == 'rect') {
                    var html = '<div>' + '<input class="key span1" type="text" value=""/>' + '<span>:</span>' + '<input class="value" type="text" value=""/>' + '</div>'
                    $(this).before(html);
                }
            });
