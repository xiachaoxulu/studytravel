  ...
  <div class="roster-page">
    <header class="ui-module-navbar pos-topbar-fixed">
      <div class="ui-module-navbar-content">
        <div class="pull-left">
          <h4><strong>员工转正</strong></h4>
        </div>
        <div class="input-group search">
          <input v-model="searchFor.userName" class="form-control">
          <span class="input-group-btn">
                    <button @click="refreshTable" class="btn btn-primary" __code="org-regular-search/搜索">搜索</button>
          </span>
        </div>
        <button type="button" class="ui-tool-button" __code="org-regular-filter/筛选" @click="choose">
      ...
      <main class="main-content">
        <div class="toolbar">
          ...
          <div class="tool" role="group">
            <button type="button " class="ui-tool-button" @click="add" __code="org-regular-add/添加">
              <span class="_name pull-left">添加</span>
            </button>
            <button type="button " class="ui-tool-button" @click="update" __code="org-regular-edit/修改">
              <span class="_name pull-left">编辑</span>
            </button>
            <button type="button " class="ui-tool-button" @click="submit" __code="org-regular-submit/提交">
              <span class="_name pull-left">提交</span>
            </button>
            <button type="button " class="ui-tool-button" @click="delete" __code="org-regular-del/删除">
            ...
            <v-dropdown class="ui-tool-dropdown" trigger="click">
              ...
              <ul class="dropdown-menu">
                <li  __code="org-regular-import/导入">
                  <a href="javascript:void(0)">导入</a>
                </li>
                <li __code="org-regular-export/更多">
                  <a href="javascript:void(0)" @click="exportExcel" >导出</a>
                </li>
                <li __code="org-regular-print/打印">
        ...
        <ui-grid v-ref:table class="testtable" :total.sync="total" :url="url" :append-params="searchFor" selectable="multi" :fields="fields" :perpage="30" :pages="[10,20,30,50]" :btrefresh="true" checked="multi" :selected.sync="selected" height="auto" table-name="officeTable" :column-config="$root.columnConfig"></ui-grid>
      </main>
...
export default {
  data() {
      return {
        url: 'api.OrgEmployee.getOffices', //查询转正url
        what: '',
        whatShow: false,
    ...
    methods: {
      ...
      exportExcel() {
        ...
        this.$root.downloadFile('acewill/employee/selectOfficialExcel.htm',param);
      },
      //筛选回调
      chooseCallBack(data) {
      ...
      submit() {
        ...
        store.actions.submitOffice(self.selected, function(data) {
      ...
      confirmed() {
        var self = this;
        store.actions.deleteOffice(self.selected, function(data) {
          self.refreshTable(); //刷新表格
        }, function(error) {
          self.$root.showTip({
            type: 'danger',
