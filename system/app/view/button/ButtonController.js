Ext.define('Web.view.button.ButtonController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.buttoncontroller',
    requires: ['Web.config.Tool', 'Web.config.Config', 'Web.view.button.ButtonAddOrUpdate'],
    //右键node节点事件
    ...
    afterRender: function() {
        ...
        store.on('beforeload', function(store, options) {
            var btnName = self.lookupReference('btnName').getValue();
    ...
    onbtnAddclick: function() {
        var self = this;
        new Web.view.button.ButtonAddOrUpdate().show().on('close', function(w, options) {
    ...
    edit: function(moduleinfo) {
        var button = this.getView().getSelectionModel().getSelection()[0].data.ID;
        var self = this;
        new Web.view.button.ButtonAddOrUpdate({
            modelID: button
        ...
        Ext.Msg.show({
            ...
            fn: function(btn) {
                if (btn === 'ok') {
                    ...
                    Web.config.Tool.ajaxPost(Web.config.Config.deleteButton, {
