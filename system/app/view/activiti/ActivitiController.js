Ext.define('Web.view.activiti.ActivitiController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.activiticontroller',
    requires: ['Web.config.Tool', 'Web.config.Config','Web.view.activiti.ActivitiAddOrUpdate'],
    //右键node节点事件
    ...
    afterRender: function() {
        var self = this;
        var url = Web.config.Tool.parseApiUrl(Web.config.Config.selectActbill);
        ...
        store.on('beforeload', function(store, options) {
            var billName = self.lookupReference('billName').getValue();
    ...
    onbtnAddclick: function() {
        var self = this;
        new Web.view.activiti.ActivitiAddOrUpdate().show().on('close', function(w, options) {
    ...
    edit: function(moduleinfo) {
        var self = this;
        new Web.view.activiti.ActivitiAddOrUpdate({
            billID: moduleinfo.ID
        ...
        Ext.Msg.show({
            ...
            fn: function(btn) {
                if (btn === 'ok') {
                    ...
                    Web.config.Tool.ajaxPost(Web.config.Config.deleteActbill,{
