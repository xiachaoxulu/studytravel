...
* @Last Modified time : 2016-11-24T16:05:00+08:00
 -->
<template>
  <div style="background: #fff;">
    ...
    <div class="wrap">
      <ui-report v-ref:report :column-total="true" :data-list="state.leaveRateEmployeeNumberReportDataList" :titles="state.leaveRateEmployeeNumberReportTitleList" :multi-titles="multiTitles" :row-total="rowTotal" :group-infos="groupColumns" :total-ignore='totalIgnore' :progress-show="processShow" :lorder="lorder" :total-formatter="totalFormatter" :td-height="70">
      </ui-report>
    </div>
...
export default {
  data() {
    ...
    methods: {
      ...
      totalFormatter(titleName, titleValue) {
        if (titleName != 'total') {
          return titleValue.substring(0, titleValue.length - 1);
        } else {
          return titleValue.toFixed(2) + '%';
        }
      },
      //选择组织类型
      orgTypeChange(type) {
      ...
      loadData(userName) {
        ...
        if (store.actions.checkConditions()) {
          if (self.state.flag) {
            ...
            store.actions.loadLeaveRateEmployeeNumberReport(function(data) {
              self.rowTotal = true;
        ...
        if (store.actions.checkConditions()) {
          if (this.state.flag) {
            ...
            this.$nextTick(function() {
                ...
                window.location.href = Vue.__Basepath + 'acewill/common/exportLeaveRateEmployeeNumberReport.htm?' + param;
              })
              // window.location.href = Vue.__Basepath + 'acewill/common/exportEmployeeNumberReport.htm?' + param;
          }
        }
      }
    }
