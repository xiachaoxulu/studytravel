...
export default {
  data() {
      return {
        ...
        url: 'api.Dashboard.getMonthCheckWork',
        ...
        fields: [{
          ...
          formater: function(value, data, index) {
            if (!isNaN(data.legalLimitTime) && !isNaN(data.allWorkTime)) {

              return ((data.legalLimitTime - 0) - (data.allWorkTime - 0)).toFixed(2);
            } else {
              return 'N/A';
            }
          },
    ...
    methods: {
      exportExcel() {
        ...
        this.$root.downloadFile('acewill/common/card/exportmonthcheckworkwarning.htm', param);
      },
      searchAtt() {
        this.refreshTable();
      },
