...
* @Last Modified time : 2016-12-02T15:59:56+08:00
 -->
  ...
  data() {
    ...
    events: {
      submit() {
        ...
        if (self.ID == "") {
          Store.actions.addOffice(self.info, function(data) {
          ...
          Store.actions.updateOffice(self.info, function(data) {
            self.officeEditCallBack();
            self.display = false;
          }, function(error) {
            self.$root.showTip({
    ...
    ready() {
      ...
      if (this.ID != "") {
        Store.actions.getOneOffice(this.ID, function(data) {
          ...
          self.info.startTime = data.startTime;
          self.info.reason = data.reason;
          console.log(self.info.reason);
