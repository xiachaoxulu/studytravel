...
* @Last Modified time : 2016-11-24T16:04:12+08:00
 -->
...
<div style="background: #fff;">
        ...
        <ul>
            <li>
                ...
                <div style="float:left;width:120px;">
                    ...
                    <option v-for="item in stateTimeCondition" :value="item.value" :selected="item.value==timeCondition">{{item.codeName}}</option>
                  </select>
                </div>
            </li>
            <li v-if="timeDay">
    ...
    <div class="wrap">
        <ui-report v-ref:report :column-total='true' :data-list="state.employeeNumberReportDataList" :titles="titles" :multi-titles="multiTitles" :row-total="rowTotal" :group-infos="groupColumns" :total-ignore='totalIgnore' :progress-show="processShow" :lorder="lorder" :td-height="70">
        </ui-report>
        ...
        loadData(userName) {
            ...
            if (store.actions.checkConditions()) {
                if (self.state.flag) {
                    ...
                    store.actions.loadEmployeeNumberReportData(function(data) {
                        self.rowTotal = true;
                        self.processShow = false;
                        self.$nextTick(() => {
                            self.$refs.report.refresh();
                            self.state.userTypeVal = []
                        })
                    }, function(error) {
                        self.processShow = false;
