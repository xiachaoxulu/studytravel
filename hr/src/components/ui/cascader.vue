<template>
  <div>
    ...
    <section class="select-box">
      <input type="text" placeholder="请选择" value="{{oneValue}}{{towValue}}{{threeValue}}" @focus="showDetail"/>
    </section>
    <section class="option-list" :class="isShow?'show-detail':''">
      <div>
        <ul>
          <li v-for="item in LevelOneMenu" @click="LevelOneMenuClick($index,item)">{{item}}</li>
        </ul>
      </div>
      <div>
        <ul>
          <li v-for="item in showLevelTowMenu" @click="LevelTowMenuClick($index,item)">{{item}}</li>
        </ul>
      </div>
      <div>
        <ul>
          <li v-for="item in showThreeMenu" @click="showText($index,item)">{{item}}</li>
        </ul>
      </div>
    </section>
...
<script>
  export default{
    ...
    methods: {
      ...
      LevelOneMenuClick(index, data){
        var str = [];
        this.curIndex = index;
        if (this.LevelTowMenu[index] != undefined) {
          for (var i = 0; i < this.LevelTowMenu[index].length; i++) {
            str.$set(str.length, this.LevelTowMenu[index][i].name);
          }
        }
        this.showLevelTowMenu = '';
        this.showThreeMenu = '';
        this.showLevelTowMenu = str;
        this.towValue = '';
        this.threeValue = '';
        this.oneValue = data + '/';
      },
      LevelTowMenuClick(index, data){
        var str = [];
        if (this.LevelTowMenu[this.curIndex] != undefined) {
          for (var i = 0; i < this.LevelTowMenu[this.curIndex].length; i++) {
            if (this.LevelTowMenu[this.curIndex][index].sub) {
              if (this.LevelTowMenu[this.curIndex][index].sub[i]) {
                str.$set(str.length, this.LevelTowMenu[this.curIndex][index].sub[i].name)
              }
            } else {
              this.threeValue = '';
              this.isShow = false;
        ...
        this.showThreeMenu = str;
        this.threeValue = '';
        this.towValue = data;
        this.endData = this.oneValue + this.towValue;
      },
      showText(index, data){
        this.threeValue = '/' + data;
        this.isShow = false;
        this.endData = this.oneValue + this.towValue + this.threeValue;
      }
    },
    ready(){
      var str = [
        {"name": "请选择", "sub": [{"name": "请选择"}], "type": 1}, {
          ...
          "sub": [{"name": "请选择", "sub": []}, {
            ...
            "sub": [],
            "type": 0
          }, {"name": "琼海", "sub": [], "type": 0}, {"name": "儋州", "sub": [], "type": 0}, {
            "name": "文昌",
            "sub": [],
            "type": 0
          }, {"name": "万宁", "sub": [], "type": 0}, {"name": "东方", "sub": [], "type": 0}, {
            "name": "澄迈县",
            "sub": [],
            "type": 0
          }, {"name": "定安县", "sub": [], "type": 0}, {"name": "屯昌县", "sub": [], "type": 0}, {
            "name": "临高县",
            "sub": [],
            "type": 0
          }, {"name": "白沙黎族自治县", "sub": [], "type": 0}, {"name": "昌江黎族自治县", "sub": [], "type": 0}, {
            "name": "乐东黎族自治县",
            "sub": [],
            "type": 0
          }, {"name": "陵水黎族自治县", "sub": [], "type": 0}, {
            "name": "保亭黎族苗族自治县",
            "sub": [],
            "type": 0
          }, {"name": "琼中黎族苗族自治县", "sub": [], "type": 0}, {"name": "其他"}],
          "type": 1
        }, {
          "name": "甘肃",
          "sub": [{"name": "请选择", "sub": []}, {
          ...
          "type": 0
        }, {"name": "海外", "sub": [{"name": "请选择"}, {"name": "其他"}], "type": 0}, {"name": "其他"}];
      for (var i = 0; i < str.length; i++) {
        this.LevelOneMenu.push(str[i].name);
        if (str[i].sub) {
          this.LevelTowMenu[i] = str[i].sub.concat([]);
        }
      }
    }
  }
...
<style>
  ...
  .option-list div ul li {
    font-size: 10px;
    padding: 7px 16px;
  }

  .option-list div ul li:hover {
    background: #eaf8fe;
  }
</style>
