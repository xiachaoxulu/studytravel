  ...
  <div class="roster-page">
    <header class="ui-module-navbar pos-topbar-fixed">
      <div class="ui-module-navbar-content">
        <div class="pull-left">
          <h4><strong>员工离职</strong></h4>
        </div>
        <div class="input-group search">
          <input v-model="searchFor.userName" class="form-control">
          <span class="input-group-btn">
                    <button @click="refreshTable" class="btn btn-primary" __code="org-dimission-search/搜索">搜索</button>
          </span>
        </div>
        <button type="button" class="ui-tool-button" __code="org-dimission-filter/筛选" @click="choose">
    ...
    <section class="warp">
      <main class="main-content">
        <div class="toolbar">
          ...
          <div class="tool" role="group">
            <button type="button " class="ui-tool-button" @click="add" __code="org-dimission-add/添加">
              <span class="_name pull-left">添加</span>
            </button>
            <button type="button " class="ui-tool-button" @click="update" __code="org-dimission-edit/编辑">
              <span class="_name pull-left">编辑</span>
            </button>
            <button type="button " class="ui-tool-button" @click="submit" __code="org-dimission-submit/提交">
              <span class="_name pull-left">提交</span>
            </button>
            <button type="button " class="ui-tool-button" @click="delete" __code="org-dimission-del/删除">
            ...
            <v-dropdown class="ui-tool-dropdown" trigger="click">
              ...
              <ul class="dropdown-menu">
                <li __code="org-dimission-import/导入">
                  <a href="javascript:void(0)" >导入</a>
                </li>
                <li  __code="org-dimission-export/导出">
                  <a href="javascript:void(0)" @click="exportExcel">导出</a>
                </li>
                <li __code="org-dimission-print/打印">
        ...
        <ui-grid v-ref:table class="testtable" :total.sync="total" :url="url" :append-params="searchFor" selectable="multi" :fields="fields" :perpage="30" :pages="[10,20,30,50]" :btrefresh="true" checked="multi" :selected.sync="selected" height="auto" table-title="员工离职" table-name="dismissionTable" :column-config="$root.columnConfig"></ui-grid>
      </main>
      ...
      <app-employee-choose :display.sync="chooseShow" :displayuserstate="false" :choose-call-back="chooseCallBack"></app-employee-choose>
    </section>
  </div>
</template>
<script>
...
export default {
  data() {
      return {
        url: 'api.OrgEmployee.getDimission', //查询转正url
        what: '',
        whatShow: false,
    ...
    methods: {
      ...
      exportExcel() {
        ...
        this.$root.downloadFile('acewill/employee/selectLeaveofficeExcel.htm',param);
      },
      //筛选回调
      chooseCallBack(data) {
      ...
      submit() {
        ...
        store.actions.submitDimission(self.selected, function(data) {
      ...
      confirmed() {
        var self = this;
        store.actions.deleteDimission(self.selected, function(data) {
          self.refreshTable(); //刷新表格
        }, function(error) {
          self.$root.showTip({
            type: 'danger',
